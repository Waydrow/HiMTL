...
if __name__ == "__main__":
	...
	train, test, train_model_input, test_model_input, dnn_feature_columns, no_ad_features = read_data_v1(task_type)

	print('train len: %d\ttest_len: %d' % (train.shape[0], test.shape[0]))

	print('Start train SGC_v1')



	# 4.Define Model,train,predict and evaluate
	model = SGC_v1(dnn_feature_columns, task_types=['binary', 'binary', 'binary'],
				task_names=['label_clk', 'label_cate', 'label_customer'], shared_expert_num=2, specific_expert_num=2,
				   no_ad_features=no_ad_features)
